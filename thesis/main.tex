...
\acknowledgements{Rád bych poděkoval především vedoucímu mé závěrečné práce Ing. Adamu Šenkovi za vstřícný přístup a cenné podněty k~teoretické i praktické části této práce. Dále bych chtěl poděkovat Marku Dostálovi a Filipu Habartovi za četné rady a korekturu textu a také rodině a přátelům za jejich podporu v průběhu mého studia.  }
...
\subsection{Follower}
	...
	Followerem neboli \uv{sledujícím} jiného uživatele se člověk stane jednoduše kliknutím na tlačítko Follow na profilu vybraného uživatele. To způsobí právě zobrazování zpráv sledovaného uživatele na hlavní stránce Twitteru. Každý uživatel si může zobrazit seznam uživatelů kteří ho sledují. 
...
\bibitem{mapreduce-img}MapReduce introduction. \textit{Computer Science} [online]. [cit. 2016-04-16]. Dostupné z:~http://www.cs.uml.edu/~jlu1/doc/source/report
/MapReduce.html
...
\begin{figure}
	\dirtree{%
		...
		.2 readme.txt\DTcomment{Instrukce pro spuštění webserveru}.
		.1 stream\_analysis\DTcomment{Zdrojové kódy pro systém na analýzu tweetů}.
		.2 Tweets\_stream\_analysis-assembly-0.1.jar\DTcomment{Jar pro spuštění}.
		.2 install.txt\DTcomment{Instrukce pro přípravu systému}.
		.2 readme.txt\DTcomment{Instrukce pro spuštění}.
